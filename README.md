...
hyp ai-rules    # ⚡ Essential — generates AGENTS.md, GEMINI.md, Copilot, Cursor, Windsurf configs
```
...
> **`hyp ai-rules` is not optional.** It generates config files for every major AI coding tool. Without them, AI starts from scratch every session and wastes thousands of tokens. **Run it once after project creation, and again after adding new routes.**

### Three project types, one shared core

```
hyp new myapp           → Web (HTTP API + Schema routes)
hyp new cli mytool      → CLI (Cobra commands)
...
```bash
# Project Creation
hyp new <name>                   # Full-stack web project
hyp new cli <name>               # CLI tool (Cobra)
hyp new desktop <name>           # Desktop app (Fyne GUI)
hyp api <name>                   # API-only web project
...
# Code Generation (CLI)
hyp generate command <name>      # Cobra subcommand

# Code Generation (Desktop)
hyp generate view <name>         # Fyne GUI view
