...
HypGo 是一個現代化的 Go Web 框架，提供 HTTP/2 和 HTTP/3 支援、Bun ORM 整合、訊息佇列和進階 JSON 驗證功能。HTTP/3.0 的速度比 HTTP/1.1 快近 10 倍。在我的使用案例中，這是非常有用且重要的，因此我在2025年開始，嘗試設計了這個框架。

這個框架具有強大的插件系統，允許您透過簡單的 CLI 命令添加 Kafka、RabbitMQ、Cassandra 等功能。它還包括自動 Docker 映像建構、熱重載開發和零停機部署功能。

## 開發故事

作為一名在全球平台工作的後端工程師，我面臨著一個關鍵挑戰：我們的亞洲客戶在訪問美國伺服器時遇到了顯著的延遲。簡單的 API 呼叫平均回應時間超過 700ms，圖片載入時間延長到數秒，使用者體驗受到嚴重影響，直接影響了我們的轉換率。
...
這些結果太重要了，希望可以公開。跨境延遲影響著全球數百萬個應用程式，開發者不應該從頭開始建構 HTTP/3 支援。HypGo 源於現實世界的痛點，並提供現實世界的結果。
