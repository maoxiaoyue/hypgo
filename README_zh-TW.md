...
**第一個讓 AI 成為一等開發者的 Go 框架 — Web（HTTP/1-3）/ CLI / 桌面 / gRPC。**
...
hyp ai-rules    # ⚡ 必要 — 生成 AGENTS.md、GEMINI.md、Copilot、Cursor、Windsurf 配置
```
...
> **`hyp ai-rules` 不是可選的。** 它為所有主流 AI 工具生成配置檔。沒有它們，AI 每次都從零開始，浪費數千 tokens。**建立專案後跑一次，新增路由後再跑一次。**

### 三種專案類型，一個共用核心

```
hyp new myapp           → Web（HTTP API + Schema 路由）
hyp new cli mytool      → CLI（Cobra 命令）
hyp new desktop myapp   → Desktop（Fyne GUI）
...
```bash
# 專案建立
hyp new <名稱>                   # 全棧 Web 專案
hyp new cli <名稱>               # CLI 工具（Cobra）
hyp new desktop <名稱>           # 桌面應用（Fyne GUI）
hyp api <名稱>                   # API-only Web 專案
...
# 程式碼生成（CLI）
hyp generate command <name>      # Cobra 子命令

# 程式碼生成（Desktop）
hyp generate view <name>         # Fyne GUI 視圖
