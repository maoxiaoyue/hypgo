...
> **`hyp ai-rules` 不是可选的。** 它为所有主流 AI 工具生成配置文件。没有它们，AI 每次都从零开始，浪费数千 tokens。**创建项目后跑一次，新增路由后再跑一次。**
...
```bash
# 项目创建
hyp new <名称>                   # 全栈 Web 项目
hyp new cli <名称>               # CLI 工具（Cobra）
hyp new desktop <名称>           # 桌面应用（Fyne GUI）
hyp api <名称>                   # API-only Web 项目
...
# 代码生成（Desktop）
hyp generate view <name>         # Fyne GUI 视图
